...
def PathCoord(path, upperPoints, lowerPoints, finalMinCord, Thickness):
    ...
    #Contour[2*len(path)+2]   = [upper[0][0],upper[1][0],Thickness]
    #Contour[2*len(path)+3] = [lower[0][0],lower[1][0],    0    ]


    #Contour[2*len(path)+3] = [upper[0][len(upper)-1],upper[1][len(upper)-1],Thickness]
    #Contour[2*len(path)+2] = [lower[0][len(lower)-1],lower[1][len(lower)-1],    0    ]
